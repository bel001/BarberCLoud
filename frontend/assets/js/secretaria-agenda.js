const sessionAgendaGlobal = AUTH.requireSession();

function mostrarIdentidad() {
  const nombre = sessionAgendaGlobal.name || sessionAgendaGlobal.email || "Secretaria";
...
async function cargarAgendaGlobal() {
  const grid = document.getElementById("agendaGlobalGrid");
  const fechaFiltro = document.getElementById("filtroFecha").value;

  try {
    const [agendaData, disponibilidadData] = await Promise.all([
      API.get("/secretaria/agenda", sessionAgendaGlobal.token),
...
document.addEventListener("DOMContentLoaded", () => {
  mostrarIdentidad();
  cargarAgendaGlobal();
  document.getElementById("filtroFecha").addEventListener("change", cargarAgendaGlobal);
});
